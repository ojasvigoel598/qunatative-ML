...
- **AI + ML hybrid agent** — `agent_sim/ai_agent.py` wraps an AI risk-desk layer around the ML agent: it vetoes longshots and bets where our price doesn't beat the sharp line (expected CLV ≥ 0), shrinks stakes where the ML disagrees most with the market (the winner's curse zone), and upscales only when sharp money agrees. Same model, same data, head-to-head (`demo/simulation_ai_ml.py`): pooled unit ROI **≈ +3.8% for AI+ML vs ≈ −3.7% for ML-only** across 10 league-seasons in I1/E0/SP1, trained on just 5 prior seasons. Full write-up: [`docs/14_ai_ml_agent.md`](docs/14_ai_ml_agent.md).
