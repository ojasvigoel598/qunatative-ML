...
- **AI + ML hybrid agent** — `agent_sim/ai_agent.py` wraps an AI risk-desk layer around the ML agent: it vetoes longshots and bets where our price doesn't beat the sharp line (expected CLV ≥ 0), shrinks stakes where the ML disagrees most with the market (the winner's curse zone), and upscales only when sharp money agrees. Same model, same data, head-to-head (`demo/simulation_ai_ml.py`), double-verified with 5 training seeds × 2 test seasons × 3 leagues (476 bets) + bootstrap CIs: pooled unit ROI **+4.7% for AI+ML vs −14.2% for ML-only**, and — the strongest evidence — the AI layer **beats the closing line by +1.50% per bet (CI [+0.32, +2.73], significant)** vs −0.04% for ML-only. E0 survives 2% exchange commission (+49.2% net, significant). Full write-up: [`docs/14_ai_ml_agent.md`](docs/14_ai_ml_agent.md); audit trail of what is real vs artifact: [`docs/15_verification_report.md`](docs/15_verification_report.md).
...
![Backtest analysis](backtests/results/backtest_analysis_ml_rl.png)
